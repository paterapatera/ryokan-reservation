...
describe("従業員管理/従業員作成", function () {
    beforeEach(function () {
        ...
        $this->service = new Service($this->builder, new WhenCreate($this->repository), $this->repository);
    });
    ...
    describe("前提：IDと暗号化パスワードは固定値で生成", function () {
        ...
        context("条件：従業員{名前:'test', Eメール:'test2@test.com', パスワード:xxxxxx}の登録", function () {
            beforeEach(function () {
                $this->service->run(new Input(
                    'test',
                    'test2@test.com',
                    'xxxxxx'
                ));
